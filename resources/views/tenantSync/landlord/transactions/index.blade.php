...
<div id="ledger">
	<div class="row">
		<div class="col-sm-12 card">
			...
			<div class="col-sm-3 card-column">
				<p class="text-center">Net Income</p>
				<p class="stat text-success text-center">{{ array_sum($landlord->transactions->pluck('amount')->toArray()) }}</p>
			</div>
			<div class="col-sm-3 card-column">
			...
			<div class="col-sm-3 card-column">
				...
				<p class="stat text-warning text-center">30%</p>
			</div>
		</div>
	</div>
	...
	<!--	<div class="col-sm-6">
			<table class="table table-striped">
				<h3 class="card-header m-t-0">
					Expenses<button @click=" generateModal(), modal.expense = true" class="btn btn-clear text-primary p-y-0"><h3 class="m-a-0 icon icon-plus"></h3></button>
				</h3>
				<thead>
					<th>Amount</th>
					<th>Description</th>
				...
				<tbody>
					<tr v-for="expense in expenses">
						...
						<td><button @click=" generateModal( expense.id )" class="btn btn-clear"><span class="text-primary icon icon-edit"></span></button></td>
						<td><button @click=" deleteTransaction( expense.id)" class="btn btn-clear"><span class="text-danger icon icon-cross"></span></button></td>
					</tr>
				</tbody>
			</table>
		</div>
		...
		<div class="card row">
			<div class="col-sm-12">
					<h3 class="card-header">
						Income<button @click=" generateModal(), modal.expense = false" class=" btn btn-clear text-primary p-y-0"><h3 class="m-a-0 icon icon-plus"></h3></button>
					</h3>
					...
					<div class="table-body table-striped">
						<div v-for="income in incomes | orderBy 'date' -1" class="table-row row">
							<div class="col-sm-2 text-success"><strong>@{{ income.amount }}</strong></div>
							<div class="col-sm-6 overflow-hide">@{{ income.description }}</div>
							<div class="col-sm-2">@{{ (income.date.substring(5) + '/' + income.date.substring(2, 4)).replace('-', '/') }}</div>
							<div class="col-sm-2">
								<button @click=" generateModal( income.id )" class="btn btn-clear p-a-0"><span class="text-primary icon icon-edit"></span></button>
								<button @click=" deleteTransaction( income.id )" class="btn btn-clear p-y-0 p-r-0"><span class="text-danger icon icon-cross"></span></button>
							</div>
						</div>
					</div>
			</div>
		...
		<div class="col-sm-6 p-l-md">
			<div class="card row">
				<div class="col-sm-12">
					...
					<div class="table-body table-striped">
						<div v-for="expense in expenses" class="table-row row">
							<div class="col-sm-2 text-danger"><strong>@{{ expense.amount }}</strong></div>
							<div class="col-sm-6 overflow-hide">@{{ expense.description }}</div>
							<div class="col-sm-2">@{{ (expense.date.substring(5) + '/' + expense.date.substring(2, 4)).replace('-', '/') }}</div>
							<div class="col-sm-2">
								<button @click=" generateModal( expense.id )" class="btn btn-clear p-a-0"><span class="text-primary icon icon-edit"></span></button>
								<button @click=" deleteTransaction( expense.id )" class="btn btn-clear p-y-0 p-r-0"><span class="text-danger icon icon-cross"></span></button>
							</div>
						</div>
					</div>
				</div>
